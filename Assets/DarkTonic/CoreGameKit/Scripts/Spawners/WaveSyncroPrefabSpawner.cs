...
public class WaveSyncroPrefabSpawner : MonoBehaviour {
    ...
    void Update() {
        ...
        StartNextEliminationWave();

        if (_waveFinishedSpawning
            || (Time.time - _waveStartTime < _currentWave.WaveDelaySec.Value)
            || (Time.time - _lastSpawnTime <= _singleSpawnTime && _singleSpawnTime > Time.deltaTime)) {
    ...
    private Transform SpawnOne(bool fromExternalScript = false) {
        ...
        if (prefabToSpawn == null) {
            if (fromExternalScript) {
                LevelSettings.LogIfNew("Cannot 'spawn one' from spawner: " + _trans.name + " because it has either no settings or selected prefab to spawn for the current wave.");
                return null;
            }
        ...
        var spawnedPrefab = PoolBoss.SpawnInPool(prefabToSpawn,
            spawnPosition, GetSpawnRotation(prefabToSpawn, _countSpawned));
        ...
        if (_currentWave.enableLimits) {
            _currentRandomLimitDistance = Random.Range(-_currentWave.doNotSpawnRandomDist.Value, _currentWave.doNotSpawnRandomDist.Value);
        }
    ...
    protected virtual Vector3 GetSpawnPosition(Vector3 pos, int itemSpawnedIndex) {
        if (_currentWave.positionXmode == WaveSpecifics.PositionMode.CustomPosition) {
            pos.x = _currentWave.customPosX.Value;
        }

        if (_currentWave.positionYmode == WaveSpecifics.PositionMode.CustomPosition) {
            pos.y = _currentWave.customPosY.Value;
        }

        if (_currentWave.positionZmode == WaveSpecifics.PositionMode.CustomPosition) {
    ...
    protected virtual Quaternion GetSpawnRotation(Transform prefabToSpawn, int itemSpawnedIndex) {
        ...
        if (_currentWave.enableRandomizations && _currentWave.randomXRotation) {
            ...
            if (_currentWave.enableKeepCenter) {
                euler.x += (itemSpawnedIndex * _currentWave.incrementRotX.Value - (_currentWaveSize * _currentWave.incrementRotX.Value * .5f));
            } else {
        ...
        if (_currentWave.enableRandomizations && _currentWave.randomYRotation) {
            ...
            if (_currentWave.enableKeepCenter) {
                euler.y += (itemSpawnedIndex * _currentWave.incrementRotY.Value - (_currentWaveSize * _currentWave.incrementRotY.Value * .5f));
            } else {
        ...
        if (_currentWave.enableRandomizations && _currentWave.randomZRotation) {
            ...
            if (_currentWave.enableKeepCenter) {
                euler.z += (itemSpawnedIndex * _currentWave.incrementRotZ.Value - (_currentWaveSize * _currentWave.incrementRotZ.Value * .5f));
            } else {
    ...
    private void LogAdjustments(int adjustments) {
        if (adjustments > 0) {
            Debug.Log(string.Format("Adjusted {0} wave(s) in spawner '{1}' to match new Level/Wave numbers", adjustments, name));
        }
    }
    ...
    private bool SetupNextWave(bool scanForWave, bool isRestart) {
        ...
        if (activeMode == LevelSettings.ActiveItemMode.Never) { // even in repeating waves.
            return false;
        }
        ...
        if (scanForWave && !isRestart) {
            ...
            switch (activeMode) {
                ...
                case LevelSettings.ActiveItemMode.IfWorldVariableInRange:
                    ...
                    for (var i = 0; i < activeItemCriteria.statMods.Count; i++) {
                        ...
                        if (min > max) {
                            LevelSettings.LogIfNew("The Min cannot be greater than the Max for Active Item Limit in Syncro Spawner '" + _trans.name + "'.");
                            return false;
                        }
            ...
            if (scanForWave && _currentWave.WaveDelaySec.Value + _currentWave.TimeToSpawnEntireWave.Value >= LevelSettings.CurrentWaveInfo.WaveDuration && LevelSettings.CurrentWaveInfo.waveType == LevelSettings.WaveType.Timed) {
            ...
            if (_currentWave.repeatWaveUntilNew && _currentWave.repeatPauseMinimum.Value > _currentWave.repeatPauseMaximum.Value) {
        ...
        if (_currentWave.repeatWaveUntilNew) {
            if (shouldInit && (LevelSettings.CurrentWaveInfo.waveType != LevelSettings.WaveType.Elimination || _currentWave.curWaveRepeatMode == WaveSpecifics.RepeatWaveMode.Endless)) { // only the first time!
            ...
            _currentWaveLength = Math.Min(_currentWaveLength, _currentWave.repeatTimeLmt.Value); // cannot exceed limits
        }
    ...
    private void CheckForValidVariablesForWave(WaveSpecifics wave) {
        ...
        wave.TimeToSpawnEntireWave.LogIfInvalid(_trans, "Time To Spawn All", wave.SpawnLevelNumber, wave.SpawnWaveNumber);
        ...
        wave.doNotSpawnIfMbrCloserThan.LogIfInvalid(_trans, "Spawn Limit Min. Distance", wave.SpawnLevelNumber, wave.SpawnWaveNumber);
        wave.doNotSpawnRandomDist.LogIfInvalid(_trans, "Spawn Limit Random Distance", wave.SpawnLevelNumber, wave.SpawnWaveNumber);
        ...
        wave.incrementPositionX.LogIfInvalid(_trans, "Incremental Distance X", wave.SpawnLevelNumber, wave.SpawnWaveNumber);
        wave.incrementPositionY.LogIfInvalid(_trans, "Incremental Distance Y", wave.SpawnLevelNumber, wave.SpawnWaveNumber);
        wave.incrementPositionZ.LogIfInvalid(_trans, "Incremental Distance Z", wave.SpawnLevelNumber, wave.SpawnWaveNumber);
        wave.incrementRotX.LogIfInvalid(_trans, "Incremental Rotation X", wave.SpawnLevelNumber, wave.SpawnWaveNumber);
        wave.incrementRotY.LogIfInvalid(_trans, "Incremental Rotation Y", wave.SpawnLevelNumber, wave.SpawnWaveNumber);
        wave.incrementRotZ.LogIfInvalid(_trans, "Incremental Rotation Z", wave.SpawnLevelNumber, wave.SpawnWaveNumber);
    ...
    private void StartNextEliminationWave() {
        ...
        if (LevelSettings.PreviousWaveInfo.waveType == LevelSettings.WaveType.Elimination &&
            ((_currentWave.curWaveRepeatMode == WaveSpecifics.RepeatWaveMode.NumberOfRepetitions && _waveRepetitionNumber + 1 >= _currentWave.repetitionsToDo.Value) || !_currentWave.repeatWaveUntilNew)) {
                ...
                LevelSettings.EliminationSpawnerCompleted(_instanceId);
            ...
            switch (_currentWave.curWaveRepeatMode) {
                ...
                case WaveSpecifics.RepeatWaveMode.UntilWorldVariableAbove:
                    ...
                    if (!allPassed) {
                        ...
                        LevelSettings.EliminationSpawnerCompleted(_instanceId); // since this never happens above due to infinite repetitions
                    }
                    break;
