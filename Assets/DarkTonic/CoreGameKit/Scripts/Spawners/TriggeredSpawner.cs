...
, uGUI
    ...
    void Update() {
        ...
        SpawnFromWaveMeta(_triggerExit2DWaveMeta, EventType.OnTriggerEnter2D);
        ...
        switch (currentWave.curRotationMode) {
            ...
            case WaveSpecifics.RotationMode.LookAtCustomEventOrigin:
                if (!currentWave.isCustomEvent) {
                    Debug.LogError("Spawn Rotation Mode is set to 'Look At Custom Event Origin' but that is invalid on non-custom event. Take a look in the Inspector for '" + name + "'.");
                    break;
                }
    ...
    private void SpawnFromWaveMeta(TriggeredWaveMetaData wave, EventType eType) {
        ...
        for (var i = 0; i < numberToSpawn; i++) {
            if (CanSpawnOne()) {
                ...
                var spawnedPrefab = SpawnWaveItem(prefabToSpawn, spawnPosition, GetSpawnRotation(prefabToSpawn, wave.countSpawned, wave));
    ...
    private bool SetupNextWave(TriggeredWaveSpecifics newWave, EventType eventType, int repetionNumber = 0) {
        ...
        var myCurrentWaveSize = Random.Range(smaller, larger + 1);
    ...
    private void LogIfCustomEventMissing(TriggeredWaveSpecifics eventGroup) {
        ...
        if (customEventName != LevelSettings.NoEventName && !LevelSettings.CustomEventExists(customEventName)) {
            LevelSettings.LogIfNew("Transform '" + name + "' is set up to receive or fire Custom Event '" + customEventName + "', which does not exist in Core GameKit.");
        }
    }
    ...
    private bool SpawnerIsPaused {
        get {
            return LevelSettings.WavesArePaused && wavePauseBehavior == WavePauseBehavior.Disable;
        }
    }

    private bool GameIsOverForSpawner {
        get {
            return LevelSettings.IsGameOver && gameOverBehavior == GameOverBehavior.Disable;
        }
    }
            ...
            switch (activeMode) {
                ...
                case LevelSettings.ActiveItemMode.IfWorldVariableInRange:
                    ...
                    for (var i = 0; i < activeItemCriteria.statMods.Count; i++) {
                        ...
                        if (min > max) {
                            LevelSettings.LogIfNew("The Min cannot be greater than the Max for Active Item Limit in Triggered Spawner '" + transform.name + "'.");
                            return false;
                        }
